...
class ICSC_hdjjc05A(ICSCWebAPI):


    def get_intention(self):
        intention = "Apply in advance to prepare for overtime."
        return intention

    def get_data_schema(self, query):
        return Object(
            id="prepare_overtime",
            description="Apply in advance to prepare for overtime information",
            examples=[
                (
            ...
            attributes=[
                Text(
                    id="from_date",
                    description="The date when the overtime will begin",
                ),
                Text(
                    id="from_time",
                    description="The time when the overtime will begin",
                ),
                Text(
                    id="to_date",
                    description="The date when the overtime will finish",
                ),
                Text(
                    id="to_time",
                    description="The time when the overtime will finish",
                ),
                Text(
                    id="ov_hrs",
                ...
                Text(
                    id="reason_desc",
                    description="description of overtime work",
                ),
                Text(
                    id="milestone",
                    description="milestone of overtime work",
                )
            ],
            many=False,
        )
    ...
    def post(self,chat_uuid, expert_data,  user: User, input_data, query):
        assert len(input_data)>0, "預申請加班 無輸入資料!"
        data = input_data[0]
        logger.debug(f"ICSC_hdjjc05A input_data {input_data}")
        from_date, from_time_input, to_date, to_time_input = convert_leave_date_time(data['from_date'],
                                                                                     data['from_time'],
                                                                                     data['to_date'],
                                                                                     data['to_time'])
        logger.debug(f"ICSC_hdjjc05A from_date, from_time_input, to_date, to_time_input {from_date, from_time_input, to_date, to_time_input}")
        ...
        # 處理無效的里程碼
        if milestone == "INVALID_MILESTONE":
            return error_msg("無效的里程碼。請使用以下里程碼之一")+"\n" + milestone_options+ "\n 你可以這樣跟我說: \n\n 預計5點到6點加班1小時，準備系統整體測試工作 A"
...
# print(guess_milestone("上線運行"))
# print(guess_milestone("設計"))
# print(guess_milestone("規劃"))
