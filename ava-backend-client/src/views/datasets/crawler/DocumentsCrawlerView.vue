...
const tableConfig = {
    ...
    allColumns: ["title", "domain", "status", "details", "crawlerStatus", "crawlerLog", "crawlerSyncLog"],
...
function goCrawlerDocumentDetail(syncId) {
    router.push(`/datasets/${props.datasets_id}/syncCrawler/${syncId}?crawlerSyncLog=false`);
}

function goCrawlerSyncLog(crawlerId) {
    router.push(`/datasets/${props.datasets_id}/syncCrawler/byDatasetId/${crawlerId}`);
}
...
<template>
    <v-container fluid>
        <div class="crawler-list">
            <SystemHeader
                title="爬蟲 - 已同步站點"
                subtitle="知識庫已同步的網站都在這裡顯示，可查詢已同步的爬蟲，每 5 秒會自動更新一次狀態。"
                ...
                <v-card-text>
                    <div class="sticky-header">
                        <div class="overflow-x-auto flex-nowrap py-2 d-flex align-center justify-space-between">
                            <div class="flex-nowrap mr-2 d-flex align-center">
                                <v-text-field
                                    v-model="searchQuery"
                                    label="搜尋網站名稱或網域"
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <!-- 同步記錄 -->
                                    <td v-else-if="column === 'crawlerSyncLog'" class="text-left">
                                        <v-btn
                                            @click.stop="goCrawlerSyncLog(item.crawlerId)"
                                            variant="text"
                                            color="accent"
                                            class="link-text"
                                        >
                                            <v-tooltip activator="parent" location="top">查看同步紀錄</v-tooltip>
