...
const getCrawlerScheduleList = async () => {
    const { data } = await axios.get(`/crawler/getCrawlerScheduleList/${datasets_id.value}?type=attachment`);
    if (data.code === 0) {
        return data.data;
    }
...
const {
    ...
    queryKey: ["crawlerAttachmentScheduleList", datasets_id],
...
async function deleteAutoSync() {
    ...
    try {
        const response = await axios.delete("/crawler/deleteCrawlerSchedule", {
            data: { datasetsId: props.datasets_id, crawlerIds: crawlerIds, type: "attachment" },
        });
    ...
    <v-container fluid>
        <div>
            ...
            <v-row v-else-if="hasNoData" class="justify-center fill-height align-center">
                <EmptyStateNoData
                    headline="尚未設定任何爬蟲附件排程"
                    text="目前還沒有設定任何爬蟲附件的自動同步排程。設定排程後，爬蟲將會按照指定的時間自動執行同步，方便您管理附件的更新。"
                />
            </v-row>
