...
import { getCrawlerAttachmentSyncList } from "@/network/service";
...
const {
    ...
    queryKey: ["crawlerAttachmentSyncList", props.datasets_id],
    queryFn: () => getCrawlerAttachmentSyncList(props.datasets_id, "attachment"),
...
const tableConfig = {
    columnLabels: {
        ...
        domain: "網域名稱",
        status: "狀態",
        attachmentsCount: "附件數量",
        details: "詳情",
        crawlerStatus: "爬蟲狀態",
        crawlerLog: "爬蟲Log",
    },
    allColumns: ["title", "domain", "status", "attachmentsCount", "details", "crawlerStatus", "crawlerLog"],
    storageKey: "crawler-site-sync-management-table-columns",
...
function goAddSource() {
    router.push(`/datasets/${props.datasets_id}/crawlerAttachmentSite`);
}

function goRouter(id) {
    router.push(`/datasets/${props.datasets_id}/syncAttachmentCrawler/${id}`);
}
...
async function toggleEnable(item) {
    ...
    try {
        const response = await axios.post("/crawler/toggleSyncCrawlerAttachmentEnable", {
            syncId,
            crawlerId,
            datasetsId,
            training_state,
...
async function handleSaveAttachments({ attachments }) {
    try {
...
<template>
    <v-container fluid>
        <div class="crawler-list">
            <SystemHeader
                title="爬蟲附件 - 已同步站點"
                subtitle="知識庫已同步的網站都在這裡顯示，可查詢已同步的爬蟲附件，每 30 秒會自動更新一次狀態。"
            ...
            <v-row v-else-if="hasNoData" class="justify-center fill-height align-center">
                <EmptyStateNoData
                    headline="尚未同步任何附件"
                    text="目前還沒有同步任何附件的資料。同步後的附件資訊將會顯示在這裡，方便您查詢和管理已爬取的附件。"
                    actionText="開始同步附件"
            ...
            <v-card class="content-card" v-else>
                <v-card-text>
                    ...
                    <v-table v-if="viewMode === 'list'" class="elevation-1 list-table">
                            ...
                            <tr>
                                ...
                                >
                                    <th
                                        :class="[
                                            'text-left',
                                            { 'text-center': ['details', 'crawlerLog'].includes(column) },
                                        ]"
                        ...
                        <tbody>
                            <template v-for="(item, index) in mutableCrawlerList" :key="item.id">
                                <tr
                                    ...
                                    >
                                        ...
                                        <td v-if="column === 'title'">
                                            ...
                                            <v-tooltip activator="parent" location="top">
                                                <span
                                                    >下次自動同步時間:
                                                    {{ formatNextSyncDate(item.next_sync_date) }}</span
                                                >
                                            </v-tooltip>
                                        </td>
                                        ...
                                        <td v-else-if="column === 'status'">
                                            <v-chip :color="getChipColor(item.training_state)">
                                                <v-tooltip activator="parent" location="top">
                                                    <span v-if="item.task_id">(Task ID: {{ item.task_id }})</span>
                                                </v-tooltip>
                                        ...
                                        <!-- 附件數量 -->
                                        <td
                                            v-else-if="column === 'attachmentsCount'"
                                            class="text-left attachment-column"
                                        >
                                            <div
                                        ...
                                        <!-- 爬蟲狀態 -->
                                        <td v-else-if="column === 'crawlerStatus'">
                                            <v-switch
                                                ...
                                                :disabled="
                                                    item.isSwitching ||
                                                    item.training_state === 1 ||
                                                    item.training_state === 2
                                                "
                                            />
                                        </td>
                                    ...
                                    <td class="text-center">
                                        <v-menu>
                                            ...
                                            <v-list>
                                                ...
                                                >
                                                    <v-list-item-title class="immediately-sync"
                                                        >立即同步</v-list-item-title
                                                    >
                                                </v-list-item>
                                            </v-list>
                                        </v-menu>
                                ...
                                <tr
                                    ...
                                    "
                                >
                                    <td :colspan="canDownloadCrawlerLog ? 9 : 8" class="pa-0">
                                        <div class="attachment-preview pa-4 bg-grey-lighten-5">
                                            ...
                                            <div class="attachment-list">
                                                ...
                                                >
                                                    <div class="d-flex align-center">
                                                        <v-icon size="small" :color="getFileIconColor(attachment.href)">
                                                ...
                                                <div class="mt-2 d-flex align-center">
                                                    <div v-if="item.attachments_select_count > 3" class="more-text">
