...
const updateModelType = "voice";
const queryKey = ["VoiceModelList", "VoiceConfigJsonb"];

const { data: modelsData } = useQuery({
    queryKey: [queryKey[0]],
    queryFn: () => fetchExpertListByModelType("voice"),
});
...
watch(
    ...
    (newValue) => {
        if (newValue && newValue[updateModelType]) {
            ...
            localVoiceModels.value = Object.entries(newValue[updateModelType])
    ...
    <v-container fluid class="pa-4">
        <v-card class="mb-6 rounded-lg kor-module-list" elevation="2">
            <v-row no-gutters class="align-center">
                <v-col cols="12" md="3" class="pa-4">
                    <h2 class="mb-0 text-blue-darken-2 text-h6 font-weight-bold">語音模組功能列表</h2>
                    <p class="mt-1 mb-0 text-blue-darken-2">管理您的語音模組</p>
                </v-col>
                <v-col cols="12" md="9">
        ...
        <v-row v-else-if="sortedAndFilteredModels.length === 0" class="justify-center fill-height align-center">
            <v-col cols="12" sm="10" md="8" lg="6" xl="4">
                <EmptyState
                    title="尚未建立語音模組"
                    description="目前還沒有建立任何語音模組，您可以建立新的模組來顯示和管理，並進一步調整模組參數。"
                />
            </v-col>
        </v-row>
        ...
        <EditModelDialog v-model:dialog="editModelDialog" :is_voice_model="true" :item="editedItem" @save="saveModel" />
