...
// 批次更新所有使用者 mutation
const batchUpdateAllMutation = useMutation({
    mutationFn: async (data) => {
        const { data: response } = await axios.put("/avaGPT/balance/batch-all", data);
    ...
    onSuccess: () => {
        emit("saved");
        emitter.emit("openSnackbar", { message: "成功更新所有使用者餘額", color: "success" });
        closeDialog();
    },
});
...
const saveBatchAllChanges = async () => {
    ...
    loading.value = true;
    try {
        await batchUpdateAllMutation.mutateAsync(updateData);
    } finally {
        loading.value = false;
    }
};
...
<template>
    <v-dialog v-model="dialog" max-width="600" persistent>
        <v-card>
            <v-card-title class="d-flex align-center bg-warning-lighten-5">
                <v-icon icon="mdi-account-group" color="warning" class="me-2" />
                <span class="text-h6">全部使用者 Balance 設定</span>
                <v-spacer />
            ...
            <v-card-text class="pt-4">
                <v-alert type="warning" variant="tonal" class="mb-4">
                    <div class="text-body-2"><strong>警告：</strong>此操作將影響系統中的所有使用者！</div>
                    <div class="mt-1 text-caption">請謹慎設定，此操作無法復原</div>
                </v-alert>

                <v-form ref="form" @submit.prevent="saveBatchAllChanges">
                    <v-row>
                        <!-- 更新選項 -->
                        <v-col cols="12">
                        ...
                        <v-col cols="12" v-if="hasAnyUpdateField">
                            <v-alert type="warning" variant="tonal" class="mb-0">
                                ...
                                <div class="mt-2 text-caption text-warning">將影響系統中的所有使用者</div>
                            </v-alert>
                        </v-col>

                        <!-- 無選擇提示 -->
                        <v-col cols="12" v-if="!hasAnyUpdateField">
                            <v-alert type="info" variant="tonal" class="mb-0"> 請至少選擇一個要更新的欄位 </v-alert>
                        </v-col>
                    </v-row>
                </v-form>
            </v-card-text>
            ...
            <v-card-actions class="px-6 pb-4 sticky-bottom">
                ...
                <v-btn color="warning" @click="saveBatchAllChanges" :loading="loading" :disabled="!hasAnyUpdateField">
                    更新所有使用者
                </v-btn>
            </v-card-actions>
        </v-card>
