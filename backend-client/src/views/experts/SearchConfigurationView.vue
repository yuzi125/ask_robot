...
const updateModelType = "search";
const queryKey = ["searchModelList", "searchConfigJsonb"];

const { data: modelsData } = useQuery({
    queryKey: [queryKey[0]],
    queryFn: () => fetchExpertListByModelType("search"),
});
...
async function saveModel(newModelValue) {
    ...
    const searchKwargs = newModelValue["search_kwargs"];
    const response = await updateExpertModelParams(expertId, modelParams, displayName, updateModelType, searchKwargs);
    if (response.code === 1) {
        emitter.emit("openSnackbar", { message: response.message, color: "error" });
    } else {
...
async function deleteModel() {
    try {
        ...
        const rs = await deleteExpertModelByName(expertId, updateModelType, displayName);
        if (rs.code === 1) {
            emitter.emit("openSnackbar", { message: rs.message, color: "error" });
        } else {
...
watch(
    ...
    (newValue) => {
        if (newValue && newValue[updateModelType]) {
            ...
            localSearchModels.value = Object.entries(newValue[updateModelType])
                ...
                .map(([uuid, data]) => ({
                    ...
                    search_kwargs: {
                        k: data.search_kwargs.k,
                        score_threshold: data.search_kwargs.score_threshold,
                        cache_threshold: data.search_kwargs.cache_threshold,
                        cache_enabled: data.search_kwargs.cache_enabled ?? true,
                        min_extra_chunk_cache_threshold: data.search_kwargs.min_extra_chunk_cache_threshold,
                        max_extra_chunk_cache_threshold: data.search_kwargs.max_extra_chunk_cache_threshold,
                    },
                    ...data.model_params,
                }));
        }
...
const sortedAndFilteredModels = computed(() => {
    return localSearchModels.value
        .filter(
            (model) =>
...
<template>
    <v-container fluid>
        <v-card class="mb-6 rounded-lg search-module-list" elevation="2">
            <v-row no-gutters class="align-center">
                <v-col cols="12" md="3" class="pa-4">
                    <h2 class="mb-0 text-blue-darken-2 text-h6 font-weight-bold">潤飾模組功能列表</h2>
                    <p class="mt-1 mb-0 text-blue-darken-2">管理您的潤飾模組</p>
                </v-col>
                <v-col cols="12" md="9">
        ...
        <v-row v-else-if="sortedAndFilteredModels.length === 0" class="justify-center fill-height align-center">
            <v-col cols="12" sm="10" md="8" lg="6" xl="4">
                <EmptyState
                    title="尚未建立潤飾模組"
                    description="目前還沒有建立任何潤飾模組，您可以建立新的模組來顯示和管理，並進一步調整模組參數。"
                />
            </v-col>
        </v-row>
        ...
        <EditModelDialog
            v-model:dialog="editModelDialog"
            :item="editedItem"
            @save="saveModel"
            :showSearchKwargs="true"
        />
