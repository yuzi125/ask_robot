...
import { fetchCacheChart } from "@/network/service";
const emitter = inject("emitter");

import CacheBar from "@/components/echarts/CacheBar.vue";
import CacheRate from "@/components/echarts/CacheRate.vue";
...
const cacheBarChartData = computed(() => {
    if (hideNoData.value) {
        const tempObj = JSON.parse(JSON.stringify(statisticalData.value.dataByDate));
        return tempObj.filter((e) => e.msgNumber || e.cacheNumber);
    } else {
        return statisticalData.value.dataByDate;
    }
});
const cacheRate = computed(() => {
    const { totalCacheNumber, totalMsgNumber } = statisticalData.value;
    const rate = totalCacheNumber / totalMsgNumber;
...
const getChartData = async (dayRange) => {
    ...
    const rs = await fetchCacheChart(reqBody);
    if (rs) {
        if (rs.data && rs.data.data) {
            statisticalData.value = rs.data.data;
        }
    } else {
        console.error("取得快取-統計資料時發生問題。");
