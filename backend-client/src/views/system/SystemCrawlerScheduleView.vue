...
const tableConfig = {
    ...
    storageKey: "system-crawler-schedule-table-columns",
    requiredFields: ["name"],
};
...
const getCrawlerScheduleList = async () => {
    const { data } = await axios.get(`/crawler/getCrawlerScheduleList`);
    if (data.code === 0) {
        return data.data;
    }
...
const {
    ...
    queryKey: ["systemCrawlerScheduleList"],
...
async function deleteAutoSync() {
    ...
    try {
        const response = await axios.delete("/system/deleteCrawlerSchedule", {
            data: { syncItems: syncItems }, // 傳遞對應的陣列
        });
