...
import { useSettingsStore, useStateStore, useUserStore } from "../store/index";
...
onMounted(() => {
    ...
    // 將該聊天室的檔案從 file server 刪除
    emitter.on("clearUploadedFiles", async (data) => {
        await clearUploadedFiles();
    });

    emitter.on("pushInput", async (data) => {
        userInput.value = data;
        user_input.value.innerText = data;
    });
    ...
    // 預設的paste不會觸發input事件，所以需要自己處理。
    user_input.value.addEventListener("paste", function (event) {
        event.preventDefault();
        let clipboardData = event.clipboardData.getData("text/plain");
        user_input.value.innerText += clipboardData;
        userInput.value = user_input.value.innerText;
        // 把光標移到最後
        const selection = window.getSelection();
        selection.selectAllChildren(user_input.value);
    ...
    const handleDocumentDragEnter = (e) => {
        if (is_tunnel.value || isTranslationExpert.value) {
            return;
        }
    ...
    if (event.ctrlKey && (event.key === "ArrowDown" || event.key === "ArrowUp")) {
        if (event.key === "ArrowUp") {
            if (recommendListActive.value > 0) {
                ...
                const targetText = recommendList.value[recommendListActive.value].text;
                userInput.value = targetText;
                user_input.value.innerText = targetText;
            }
...
watch(chatPartner, (newValue, oldValue) => {
    ...
    user_input.value.innerText = userInput.value;
...
async function pushData() {
    ...
    if (hasAttachedFiles.value) {
        emitter.emit("pushData", { data: uploadedFiles.value, text: userInput.value, type: "document_qa_tunnel" });
        // 清空檔案和輸入
        // fileUploaderRef.value.clearFiles();
        hasAttachedFiles.value = false;
    } else {
    ...
    //新增歷史訊息
    !is_tunnel.value && addHistoryMsg(userInput.value);
    //清空input與暫存input
    userInput.value = "";
    user_input.value.innerText = "";
...
async function upload_audio_file(recordedBlob) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            user_input.value.innerText = text;
            user_input.value.focus();
        } else {
...
function handleTagClick(tag) {
    ...
    if (lastClickedTagName === tag.name && userInput.value === tagContent) {
        ...
        user_input.value.innerText = tagContent;
...
// 假的光標特效(特殊處理，因為原生光標在沒有輸入值時不會垂直置中)
const showFakeCaret = ref(true);
watch(
    () => userInput.value,
    async () => {
        await nextTick();
        if (userInput.value && userInput.value.trim().length > 0) {
            showFakeCaret.value = false;
            user_input.value.style.caretColor = "black";
        } else {
            showFakeCaret.value = true;
            user_input.value.style.caretColor = "transparent";
        }
    }
);
...
const handleUploadFiles = async (files) => {
    ...
    try {
        ...
        if (successfulUploads.length > 0) {
            // 保存新上傳成功的檔案，合併到現有檔案列表
            const currentFileIds = uploadedFiles.value
                .filter((f) => !files.some((newFile) => newFile.id === f.id)) // 排除剛上傳的檔案
                .map((f) => f.id);

            // 合併已有檔案和新上傳的檔案
            uploadedFiles.value = [
                ...uploadedFiles.value.filter((f) => !files.some((newFile) => newFile.id === f.id)), // 保留原有不在此次上傳中的檔案
                ...successfulUploads, // 添加新上傳成功的檔案
            ];
...
<template>
    ...
    <Teleport to="#global-teleport-target">
        <div
            v-if="isDraggingGlobally && !is_tunnel && !isTranslationExpert && chat_file_upload_enable"
    ...
    <form class="input_msg" @submit.prevent="pushData">
        ...
        <div class="input_box">
            <div class="left" v-if="showFakeCaret">
                <template v-if="!is_tunnel">
                    ...
                    >
                        <p><span class="fakeCaret"></span>{{ item }}</p>
                    </div>
                </template>
            ...
            ></div>
            <div class="relative z-50 bottom-4" v-if="enable_rtc_recording == 1">
            ...
            <div
                class="relative z-50 bottom-4"
                :class="{ 'right-[20px]': enable_rtc_recording == 1 }"
                v-if="!is_tunnel && !isTranslationExpert && chat_file_upload_enable"
            >
                <span class="right" @click="toggleFileUploader"><i class="fa-solid fa-paperclip"></i></span>
            </div>
            <div :class="{ 'mr-[25px]': enable_rtc_recording == 1 }" class="right">
                <Button
                    v-if="is_tunnel"
                    variant="destructive"
                    size="sm"
                    @click="showConfirmDialog = true"
                    class="h-5 p-3 text-xs font-medium"
                >
        ...
        <button
            ...
            ref="send_msg"
            type="submit"
            :disabled="openAudio || isWaitRes || (!userInput && !hasAttachedFiles)"
            :style="(userInput || hasAttachedFiles) && !openAudio && !isWaitRes ? { opacity: '1' } : { opacity: '0.5' }"
        >
    ...
    <Dialog :open="showConfirmDialog" @update:open="updateDialogState">
        <DialogContent class="sm:max-w-[425px] rounded-lg border-none shadow-lg">
            <DialogHeader class="pb-2 space-y-2 text-left">
                <DialogTitle class="text-xl font-semibold tracking-tight">確認退出問答模式?</DialogTitle>
                <DialogDescription class="text-sm text-muted-foreground">
                    <div class="mt-1 space-y-2">
                        <p>離開後將會：</p>
                        <ul class="pl-5 space-y-1 list-disc">
                            <li>結束目前的問答模式</li>
                            <li>刪除此次上傳的所有檔案(如有)</li>
                            <li>無法再繼續針對這些檔案提問(如有)</li>
                            <li>無法下載這些檔案(如有)</li>
                        </ul>
                    </div>
                </DialogDescription>
            ...
            <!-- <div class="p-3 mt-2 mb-3 text-sm border rounded-md bg-muted/30 border-muted">
                <div class="flex items-start">
                    <i class="mt-0.5 mr-2 fa-solid fa-circle-info text-primary"></i>
                    <p>檔案資料會立即從系統中移除，此操作不可逆。</p>
                </div>
            </div> -->

            <DialogFooter class="flex justify-end gap-2 mt-2">
                <Button variant="outline" @click="showConfirmDialog = false" class="min-w-[80px]"> 取消 </Button>
                <Button variant="destructive" @click="cancelform" class="min-w-[80px]">
                    <i class="mr-1.5 fa-solid fa-check"></i>確認退出
                </Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
...
.input_msg {
    ...
    .input_box {
        ...
        .user_input {
            ...
            min-height: 2.5rem;
            display: flex;
            align-items: center; /* 確保輸入內容垂直置中 */
            justify-content: flex-start; /* 讓輸入從左開始 */
