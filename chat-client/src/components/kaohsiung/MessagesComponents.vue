<script setup>
import { computed, inject, nextTick, onMounted, ref, toRefs, watch, onUnmounted } from "vue";
...
import { useUserStore, useStateStore, useSettingsStore, useApiStatusStore } from "@/store/index";

import { useVirtualizer } from "@tanstack/vue-virtual";
import { useQuery, useQueryClient, keepPreviousData } from "@tanstack/vue-query";
import VueEasyLightbox from "vue-easy-lightbox";

import CardsComponents from "@/components/kaohsiung/CardsComponents.vue";
import EchartsLineComponents from "@/components/EchartsLineComponents.vue";
import EchartsStackedComponents from "@/components/EchartsStackedComponents.vue";
import FullImageComponents from "@/components/FullImageComponents.vue";
import IframeComponent from "@/components/IframeComponents.vue";
import EchartsComponents from "@/components/EchartsComponents.vue";
import FormComponents from "@/components/FormComponents.vue";
import ExpandableQaComponent from "@/components/ExpandableQaComponent.vue";
import RecheckComponents from "@/components/RecheckComponents.vue";
import HtmlJsonComponents from "@/components/kaohsiung/HtmlJsonComponents.vue";
import SourceChunkDrawer from "@/components/messages/SourceChunkDrawer.vue";
import ExtraChunkComponents from "@/components/messages/ExtraChunkComponents.vue";
import FloatingDateDisplay from "@/components/FloatingDateDisplay.vue";
...
const scrollType = ref(0); // 0:滾動到新CHAT置頂； 1:不滾動。
...
const {
    ...
    staleTime: Infinity, // 设置为 Infinity，除非手动调用 refetch 否则不会重新请求
...
onMounted(async () => {
    ...
    emitter.on("pushData", async (msg) => {
        ...
        pushData_msg = msg;
        emitter.emit("checkInsideIframe");
...
const displayMessages = computed(() => {
    ...
    if (pageData.value?.messages) {
        const newMessages = MsgConvertRender(pageData.value.messages);

        // 先合併歷史訊息
        if (currentPage.value > 0) {
            result = [...newMessages, ...allHistoryMessages.value];
        } else {
            result = [...allHistoryMessages.value, ...newMessages];
        }
        ...
        result.forEach((msg) => {
            // 使用更可靠的唯一键
            const key = msg.time || msg.history_message_id || msg.messageId;
            messageMap.set(key, msg);
        });

        // 無論在哪個頁面，都確保包含本地訊息和待處理訊息
        // 這是關鍵修改 - 不再僅在 currentPage === 0 時處理本地訊息
        const localMessages = tempMessages.value.length > 0 ? tempMessages.value : allMessages.value;
        ...
        if (welcomeMessage.value && welcomeInitialTime.value) {
            ...
            if (insertIndex !== -1) {
                result.splice(insertIndex, 0, welcomeMessage.value);
            } else {
                result.push(welcomeMessage.value);
            }
        }
...
watch(chatPartner, (newValue, oldValue) => {
    // console.log("chatPartner", newP, oldP);
    // 為了 iframe 重新整理不要跑到其他聊天室所設定
    // console.log("newp", newP.roomId);
...
async function getContactList() {
    //這邊之後不用uid,token放header 後端解密出uid
    if (!isIframe) {
...
async function updateOldData(msg, time, type, is_steaming) {
    ...
    if (!time && !type) {
        ...
        //更新local
        let localMsg = getLocalMsg(roomId.value);
        let localLastMsg = localMsg.find((f) => f.time == botLastMsg.time);
        localLastMsg.message.push(msg);

        //更新資料庫
        let rs = await axios.post("/bot/updateMessage", {
            message: localLastMsg,
            roomId: roomId.value,
        });
        // if (rs.data.code === 1) {
        ...
        let lastMsg = messages.value[messages.value.length - 1]; // 最後一則訊息
        let lastMessage = lastMsg.message[lastMsg.message.length - 1]; // 最後一則訊息的組件內容
        // console.log("lastMsg: ", lastMsg);
        // console.log("lastMessage: ", lastMessage);
        let message = messages.value.find((f) => f.sender == "bot" && f.time == time); // 機器人的最後一則訊息
        // console.log("before message: ", message);
        //更新local
        let localMsg = getLocalMsg(roomId.value);
        // console.log("localMsg: ", localMsg);
        let localLastMsg = localMsg.find((f) => f.time == time);
        let localLastMsgIndex = localMsg.findIndex((f) => f.time == time); // 找到索引
        if (msg.type) {
            let obj = {};
            obj[msg.type] = "";
            targetType = msg.type;
            switch (msg.type) {
                ...
                case "card":
                    ...
                    localLastMsg.message.push(obj);
                    break;

                case "html_json":
                    // console.log("html_json lastMsg: ", lastMsg);
                    // console.log("html_json localLastMsg: ", localLastMsg);
            ...
            //將更新後的localLastMsg放回localMsg
            localMsg[localLastMsgIndex] = lastMsg;
...
async function handleStreamEnd(msg, userMsgTime) {
    ...
    const botMsgTime = new Date().getTime();
...
async function scrollToQATop() {
    try {
        ...
        if (lastIndex >= 0) {
            ...
            rowVirtualizer.value.value.scrollToIndex(lastIndex, {
                align: "start",
                behavior: "smooth", // 改用 "auto" 以避免平滑滾動的潛在問題
            });
...
async function copyText(event, index) {
    ...
    const text = element.closest(".msg");

    ////////////////////////////////////////////////////////////////
    const paragraphs = Array.from(text.querySelectorAll(".mkd"));

    const textToCopy = paragraphs.map((p) => p.innerText).join("\n");
    // .slice(0, -1);
                    ...
                    >
                        ...
                        >
                            ...
                            <div v-else>
                                ...
                                <div v-for="item1 in messages[virtualRow.index].message" :key="item1" v-else>
                                    <div
...
.msgGroupBox {
    ...
    .msgGroup {
        ...
        .msgBox {
            ...
            .wait_res {
                background: url("/dots.gif") no-repeat center;
