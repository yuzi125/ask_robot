...
function confirmAction(form) {
    if (singleSelect.value === -1) {
        errorMsg.value = "*請選擇一個";
    } else {
        errorMsg.value = "";
        const fields = {
            id: card.value.id,
            singleSelect: card.value.items[singleSelect.value].value,
            select: card.value.items[singleSelect.value].value,
        };
...
const singleSelect = ref(-1);

function handleClick(item, index) {
    if (isItemDisabled(item)) return;
    if (singleSelect.value === index) {
        singleSelect.value = -1;
    } else {
    ...
    if (singleSelect.value === -1) {
        ...
        text = `${selectRoom} ${card.value.items[singleSelect.value].text}`;
    }
            ...
            <li v-for="(item, index) in card.items" :key="index" class="item">
                ...
                <div
                    ...
                    :class="[
                        ...
                        {
                            singleSelect: index === singleSelect,
                        },
                    ]"
                    @click="handleClick(item, index)"
                >
                    <span class="selected selected-single">單</span>
